...
INTERN inline bool check_phf_header( void );
INTERN inline bool read_phf_header( void );
INTERN inline bool write_phf_header( void );
...
INTERN void fprintf_conv_set( FILE* fp, huffman_conv_set* conv_set, const char* id, const char* prefix );
INTERN const char* to_ascii_string( int value );
INTERN const char* to_binary_string( unsigned int number, int len );
#endif
...
INTERN const unsigned char appversion = 2;
...
INTERN const char*  cmp_ext      = "phf";
INTERN const char*  unc_ext      = "unc";
#endif
INTERN const char   cmp_magic[] = { 'H', 'S' };
...
EXPORT bool phflib_convert_stream2stream( char* msg )
{
	...
	return phflib_convert_stream2mem( NULL, NULL, msg ); 
}
#endif
...
{
	// init streams
	phflib_init_streams( (void*) in, 0, 0, (void*) out, 0 );
...
EXPORT void phflib_init_streams( void* in_src, int in_type, int in_size, void* out_dest, int out_type )
{
	...
	if ( !force_enc ) {
		...
		if ( (buffer[0] == cmp_magic[0]) && (buffer[1] == cmp_magic[1]) ) {
			// file might be PHF - check version and header
			if ( check_phf_header() ) {
				filetype = F_PHF;
				...
				strcpy( cmpfilename, (  in_type == 0 ) ? (char*) in_src   : "PHF in memory" );
	...
	if ( filetype == F_UNK ) {
		...
		strcpy( uncfilename, (  in_type == 0 ) ? (char*) in_src   : "PHF in memory" );
...
{	
	...
	if ( developer ) {
	fprintf( msgout, "\n" );
	fprintf( msgout, " [-nfo]   dump huffman coding info\n" );
...
{	
	if ( filetype == F_UNK ) {
		switch ( action ) {
			...
			#if !defined(BUILD_LIB) && defined(DEV_BUILD)
			case A_INFO: // only for un-encoded files!
				execute( dump_info );
				break;
			#else
			default:
...
{	
	...
	// check if this is a compressed phf file
	while ( !force_enc ) {
		// read file id and version number
		if ( str_in->read( fileid, 1, 3 ) != 3 ) break;
		// check magic number
		if ( ( fileid[0] != cmp_magic[0] ) || ( fileid[1] != cmp_magic[1] ) ) break;
		// check phf header, extract file name
		if ( !check_phf_header() ) break;
		// alright: this is a .phf file!
...
INTERN bool encode_file( void )
{
	...
	huffman_code** table;
	huffman_writer* encoder;
	#if defined CHUNK_SIZE
	unsigned char chunkbyte[4];
	#endif
	unsigned char* cdata;
	unsigned int csize;
	unsigned char v;
	...
	// safety checks
	if ( uncfilesize < 0 ) {
		sprintf( errormessage, "cannot handle files >= 2GB" );
		errorlevel = 2;
		return false;
	}
	...
	if ( !write_phf_header() ) return false;
	
	
	// --- encode file ---
	// preparations
	str_in->rewind();
	ctable = build_ctable( str_in ); // build & write canonical table
	if ( ctable == NULL ) {
		sprintf( errormessage, MEM_ERRMSG );
		errorlevel = 2;
		return false;
	}
	str_out->write( (void*) ctable->data, 1, ctable->total_size );
	
	// compress data
	str_in->rewind();
	if ( ctable->len_ptr == NULL ) { // incompressible case
		while ( str_in->read( &v, 1, 1 ) == 1 )
			str_out->write( &v, 1, 1 );
	} else if ( ctable->n_symbols > 1 ) { // special cases are handled in decoder
		// convert to huffman codes
		table = convert_ctable2hcode( ctable );
		
		// main compression loop
		for ( int i = 0; i < uncfilesize; ) {
			// encode data
			encoder = new huffman_writer( 0 );
			while ( i++ < uncfilesize ) {
				str_in->read( &v, 1, 1 );
				encoder->encode_symbol( table, v );
				#if defined CHUNK_SIZE
				if ( i % CHUNK_SIZE == 0 ) break;
				#endif
			}
			// write to file
			cdata = encoder->getptr();
			csize = encoder->getpos();
			#if defined CHUNK_SIZE
			pack_bytes( csize, 4, chunkbyte );
			str_out->write( chunkbyte, 1, 4 );
			#endif
			str_out->write( cdata, 1, csize );
			delete( encoder );
			free( cdata );
		}
...
INTERN bool decode_file( void )
{
	...
	// --- read PHF header again ---
	str_in->read( (void*) &v, 1, 1 ); // skip byte 0
	str_in->read( (void*) &v, 1, 1 ); // skip byte 1
	str_in->read( (void*) &v, 1, 1 ); // skip byte 2
	if ( !read_phf_header() ) {
	...
	// --- decode file ---
	// preparations
	ctable = read_ctable( str_in ); // read canonical table
	if ( ctable == NULL ) {
		sprintf( errormessage, "errors in canonical table" );
		errorlevel = 2;
		return false;
	}
	
	// decoding
	if ( ctable->n_symbols == 1 ) { // easy case: only one symbol in table
		v = ctable->val_ptr[0];
		for ( int i = 0; i < uncfilesize; i++ )
			str_out->write( &v, 1, 1 );
	} else if ( ctable->len_ptr == NULL ) { // incompressible case
		while ( str_in->read( &v, 1, 1 ) == 1 )
			str_out->write( &v, 1, 1 );
	} else { // default case
		conv_set = convert_ctable2hconv( ctable ); // convert to conv_set
		if ( conv_set == NULL ) {
			sprintf( errormessage, MEM_ERRMSG );
			errorlevel = 2;
			return false;
		...
		for ( int i = 0; i < uncfilesize; ) {
			#if defined CHUNK_SIZE
			str_in->read( chunkbyte, 1, 4 );
			csize = unpack_bytes( 4, chunkbyte );
			#else
			csize = cmpfilesize - str_in->getpos();
			#endif
			...
			str_in->read( cdata, 1, csize );
			// decode data
			decoder = new huffman_reader( cdata, csize );
			while ( i++ < uncfilesize ) {
				v = decoder->decode_symbol( conv_set );
				str_out->write( &v, 1, 1 );
				#if defined CHUNK_SIZE
				if ( i % CHUNK_SIZE == 0 ) break;
				#endif
...
/* -----------------------------------------------
	check phf header, extract uncompressed name
	----------------------------------------------- */
INTERN inline bool check_phf_header( void )
{
...
INTERN inline bool read_phf_header( void )
{
	...
	uncfilesize = unpack_bytes( 4, nbyte );
...
INTERN inline bool write_phf_header( void )
{
	...
	pack_bytes( uncfilesize, 4, nbyte );
...
/* -----------------------------------------------
	fetches numbers from data, MSB first
	----------------------------------------------- */
INTERN unsigned int unpack_bytes( int num_bytes, unsigned char* data ) {
	unsigned int number = 0;
	
	while ( num_bytes-- > 0 )
		number |= (*data++) << (num_bytes * 8);
...
INTERN void pack_bytes( unsigned int number, int num_bytes, unsigned char* data ) {
	while ( num_bytes-- > 0 )
		(*data++) = ( unsigned char ) ( number >> (8*num_bytes) ) & 0xFF;
}
...
{	
	...
	// create filename
	fn = (char*) calloc( strlen( uncfilename ) + 9, sizeof( char ) );
	sprintf( fn, "%s_%s.%s", uncfilename, "phf", "nfo" );
	...
	// canonical table / encoding table info
	ctable = build_ctable( str_in );
	fprintf( fp, "<Huffman coding info file %s>\n\n\n", uncfilename );
	fprintf( fp, "uncompressed size: %i\n", uncfilesize );
	fprintf( fp, "# of symbols: %i\n", ctable->n_symbols );
	fprintf( fp, "max codelength: %i\n\n", ctable->max_clen );
	if ( ctable->len_ptr == NULL )
		fprintf( fp, "uniform distribution of symbols\n->no compression possible" );
	
	fprintf( fp, "Canonical lengths -> numbers of codes:\n" );
	if ( ctable->len_ptr != NULL ) for ( int i = 1; i <= ctable->max_clen; i++ )
		fprintf( fp, "%2i -> %3i\n", i, ctable->data[i] );
	else fprintf( fp, "%2i -> %3i\n", 8, 256 );
	fprintf( fp, "\n" );
	
	fprintf( fp, "Canonical symbols -> lengths (counts/bytes total):\n" );
	if ( ctable->len_ptr != NULL ) for ( int i = 0, s = 0; s < ctable->n_symbols; ) {
		for ( int n = ctable->data[++i]; n > 0; n--, s++ )
			fprintf( fp, "%02X (%s) -> %2i (%i/%i)\n",
				ctable->val_ptr[s],
				to_ascii_string( ctable->val_ptr[s] ),
				i, (int) ctable->counts[s],
				( ( i * ctable->counts[s]  + 7 ) / 8 ) );
	} else for ( int s = 0; s < 256; s++ ) fprintf( fp, "%02X (%s) -> %2i (%s)\n",
		s, to_ascii_string( s ), 8, "???" );
	fprintf( fp, "\n" );
	
	if ( ctable->len_ptr != NULL ) {
		// huffman table / encoding table info
		hcodes = convert_ctable2hcode( ctable );
		fprintf( fp, "Huffman encoding table (symbol -> code/length):\n" );
		for ( int i = 0; i < 256; i++ ) if ( hcodes[i] != NULL )
			fprintf( fp, "%02X (%s) -> %s (%X/%i)\n",
				i, to_ascii_string( i ),
				to_binary_string( hcodes[i]->code, hcodes[i]->len ),
				hcodes[i]->code, hcodes[i]->len );
		fprintf( fp, "\n" );
		// huffman table / decoding table info
		conv_set = convert_ctable2hconv( ctable );
		fprintf( fp, "Huffman decoding table (code/length -> symbol):\n" );
		fprintf_conv_set( fp, conv_set, "", "" );
	}
...
#if !defined(BUILD_LIB) && defined(DEV_BUILD)
/* -----------------------------------------------
	Dump huffman conversion table (helper)
	----------------------------------------------- */
INTERN void fprintf_conv_set( FILE* fp, huffman_conv_set* conv_set, const char* id, const char* prefix ) {
	char* new_id;
	char* new_prefix;
	...
	// table description
	if ( strlen( prefix ) > 0 )
		fprintf( fp, "ext table %s -> hwidth %i, prefix %s\n", id, conv_set->hwidth, prefix );
	else fprintf( fp, "main table %s -> hwidth %i\n", id, conv_set->hwidth );
	
	// first pass - output codes/length/translations
	for ( int n = 0, n_ext = 0; n < 1 << conv_set->hwidth; n++ ) {
		if ( conv_set->h[n]->ext == NULL )
			fprintf( fp, "%s (%02X/%i) -> %02X (%s)\n",
				to_binary_string( n, conv_set->hwidth ),
				n, conv_set->h[n]->len, conv_set->h[n]->val,
				to_ascii_string( conv_set->h[n]->val ) );
		else fprintf( fp, "%s (%02X/%i) -> ext table #%s/%i\n",
				to_binary_string( n, conv_set->hwidth ),
				n, conv_set->h[n]->len, id, n_ext++ );
	}
	
	// second pass - output ext tables
	for ( int n = 0, n_ext = 0; n < 1 << conv_set->hwidth; n++ ) {
		if ( conv_set->h[n]->ext != NULL ) {
			new_id = (char*) calloc( strlen( id ) + 1 + 3, sizeof( char* ) ); // !!!
			new_prefix = (char*) calloc( strlen( id ) + 1 + conv_set->hwidth, sizeof( char* ) );
			if ( strlen( id ) == 0 ) sprintf( new_id, "%i", n_ext++ );
			else sprintf( new_id, "%s/%i", id, n_ext++ );
			sprintf( new_prefix, "%s%s", prefix, to_binary_string( n, conv_set->hwidth ) );
			fprintf_conv_set( fp, (huffman_conv_set*) conv_set->h[n]->ext, new_id, new_prefix );
			free( new_id );
			free( new_prefix );
		}
	}	
...
#if !defined(BUILD_LIB) && defined(DEV_BUILD)
/* -----------------------------------------------
	Get ASCII representation in string (helper)
	----------------------------------------------- */
INTERN const char* to_ascii_string( int value ) {
	static char* string = (char*) calloc( 3 + 1, sizeof( char* ) );
	
	if ( ( value >= 0x20 ) && ( value < 0x7F ) )
		sprintf( string, "'%c'", value );
	else sprintf( string, "\\%02X", value );
...
#if !defined(BUILD_LIB) && defined(DEV_BUILD)
/* -----------------------------------------------
	Convert decimal number to binary string (helper)
	----------------------------------------------- */
INTERN const char* to_binary_string( unsigned int number, int len ) {
	static char* string = (char*) calloc( 32 + 1, sizeof( char* ) );
	int i;
	
	if ( len > 32 ) len = 32;
	for ( i = 0; len > 0; len-- )
		string[ i++ ] = ( ( number >> (len-1) ) & 0x1 ) ? '1' : '0';
	string[ i ] = '\0';	
